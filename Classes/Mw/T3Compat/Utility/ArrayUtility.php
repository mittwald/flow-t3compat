...
class ArrayUtility
{
    ...
    static public function filterByValueRecursive($needle = '', array $haystack = array())
    {
        ...
        $callback = function (&$value, $key) use ($needle, &$resultArray)
        {
            ...
            {
                ($subArrayMatches = self::filterByValueRecursive($needle, $value));
            ...
            function (array $a, array $b) use ($key, $ascending)
            {
                if (!isset($a[$key]) || !isset($b[$key]))
                {
                    throw new \RuntimeException(
                        'The specified sorting key "' . $key . '" is not available in the given array.', 1373727309
                    );
                }
     ...
     * @return void
     */
    static public function mergeRecursiveWithOverrule(
        array &$original,
        array $overrule,
        $addKeys = TRUE,
