...
class VersionNumberUtility
{
    ...
    static public function convertVersionNumberToInteger($versionNumber)
    {
        ...
        return (int)(((int)$versionParts[0] . str_pad((int)$versionParts[1], 3, '0', STR_PAD_LEFT)) . str_pad(
                (int)$versionParts[2],
                3,
                '0',
                STR_PAD_LEFT
    ...
    static public function convertIntegerToVersionNumber($versionInteger)
    {
        if (!is_int($versionInteger))
        {
            throw new \InvalidArgumentException(
                'TYPO3\\CMS\\Core\\Utility\\VersionNumberUtility::convertIntegerToVersionNumber() supports an integer argument only!',
                1334072223
            );
        }
    ...
    static public function convertVersionsStringToVersionNumbers($versionsString)
    {
        ...
        for ($i = 0; $i < $versionsCount; $i++)
        {
            ...
            {
                $cleanedVersion[$j] = MathUtility::forceIntegerInRange(
                    $cleanedVersion[$j],
                    0,
                    999
                );
